...
function begin_play(){
	...
	if (count == 1){
		...
		var turn_display = document.getElementById("turn_info");
		var tag;

		empty = isEmpty(player1.value);
		empty2 = isEmpty(player2.value);
		...
		if (empty || empty2){
			...
			turn_display.innerHTML = "Turn for: " + tag.bold();
		}
	}
	else{
...
function reset_play(){
	...
	if (count == 0){
		...
		player1.value = '';
		player2.value = '';
		turn_display.innerHTML = "Game has not begun";
		count = 0;
		turn = 1;
		turnCount = 0;
...
function play() {
	if (count > 0){
		...
		var found = table_ids.includes(move.value);
		var index = table_ids.indexOf(move.value, 0);
		if (taken.includes(move.value)){
			alert('Please enter a location that has not been taken.')
			return
		}
		else{
			taken.push(move.value);
		}
		var turn_display = document.getElementById("turn_info");
		var tag;
		if (found){
			...
			taken.pop();
			alert('Please enter a valid position in the table');
			return
		}
		...
		var checkArray = [
			...
			[board_state[0], board_state[3], board_state[6]],
			[board_state[1], board_state[4], board_state[7]],
			[board_state[2], board_state[5], board_state[8]],
			[board_state[0], board_state[4], board_state[8]],
			[board_state[2], board_state[4], board_state[6]]
		];
		
		for (var i = 0; i < 8; i++){
			if ((checkArray[i][0] == 1) && (checkArray[i][1] == 1) && (checkArray[i][2] == 1)){
				alert('X wins!')
				reset_play();
				return
			}
			else if ((checkArray[i][0] == 0) && (checkArray[i][1] == 0) && (checkArray[i][2] == 0)){
				alert('O wins!')
				reset_play();
				return
			}
